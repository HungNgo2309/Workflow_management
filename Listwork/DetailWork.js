...
import {IconButton, MD3Colors,Appbar, Dialog, Portal, Button, Provider, Avatar} from 'react-native-paper';
...
const DetailWork=({route,navigation})=>{
    ...
    useEffect(() => {
        fetchData();
    }, [work.Project_ID, user.uid]); // Thêm work.Project_ID và user.uid vào dependencies của useEffect
    const fetchNotify=async()=>{
        try {
            const Notify=await firestore().collection('Notification').where("Project_ID", '==', work.Project_ID);
            const list = [];
            const snapshot = await Notify.get();
            snapshot.forEach(doc => list.push(doc.data()));
            const mergedData =await list.map(list => ({
                ...list,
                tasks: dataTask.filter(tasks => list.Title==tasks.Task_name)
            }));
            await setNotify(mergedData);
            //console.log(notify[0].tasks[0])
        } catch (error) {
            //console.error("Lỗi khi lấy dữ liệu:", error);  
        }
    }
    ...
    return(
        ...
        <Appbar.Content title={work.Project_name}  />
            ...
            <Appbar.Action icon={MORE_ICON} onPress={hideDialog} />
        </Appbar.Header>
            <TabView
                ...
                />
         <Portal>
            <Dialog visible={visible} onDismiss={hideDialog} style={{position:'absolute',top:0,right:2}}>
